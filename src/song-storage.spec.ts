...
describe('edit', () => {
  it('does not change the urlName', async () => {
    const song = {name: 'name', melody: 'mel 1', urlName: 'name', lyrics: 'lyrics', _id: undefined as any}
    const result = await col.insertOne(song);
    await edit({...song, melody: 'mel 2'});
...
describe('create', () => {
  it('sets the urlName to the name', async () => {
    const song = await create({name: 'name', melody: 'mel 1', lyrics: 'lyrics'});
  ...
  describe('when creating a second song the the same name', () => {
    it('invent a nameUrl for the second one', async () => {
      const song1 = await create({name: 'name', melody: 'mel 1', lyrics: 'lyrics'});
      const created1 = await col.findOne({_id: new ObjectId(song1._id)});
      expect(created1.urlName).to.equal('name');
  
      const song2 = await create({name: 'name', melody: 'mel 1', lyrics: 'lyrics'});
